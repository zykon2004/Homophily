...
class Game:
    ...
    def introduce_behavior(
        self, players_with_behavior, behavior: BehavioralAttribute
    ) -> None:
    ...
    def behavior_propagation(self):
        players_with_behavior = sum(
            (
                1
                for player in self.phases[-1].players
                if player.behavior == self.behavior
            )
        )
        return round(
            float(players_with_behavior) * 100 / len(self.phases[-1].players), 3
        )
    ...
    def is_blue_behavior_changed(self):
        for player in self.phases[-1].players:
            if all(
                (player.group == Player.BLUE_BEHAVIOR, player.behavior == self.behavior)
            ):
                return True
        return False
    ...
    def connections_mean(self):
        return float(
            sum(len(player.connections) for player in self.players.values())
        ) / len(self.players)
    ...
    def inner_edges(self):
        return (
            len(self.red_players_connections) + len(self.blue_players_connections)
        ) / 2

    @property
    def red_players_count(self):
        return len(
            [player for player in self.players.values() if player.group.name == "Red"]
        )

    @property
    def blue_players_count(self):
        return len(
            [player for player in self.players.values() if player.group.name == "Blue"]
        )

    def __call__(self, deepcopy_players: bool = False):
        modified_players = True
        while modified_players:
            next_phase = GamePhase(self.phases[-1].players, deepcopy_players)
            modified_players = next_phase.propergate_behavior()
            if modified_players:
